...
1. Changes made in search-pv.R
  ...
  new  
   ...
   10. The API now returns a few HATEOAS (Hypermedia as the Engine of Application State) links to retrieve more information, example "inventor": "https://search.patentsview.org/api/v1/inventor/252373/" Added retrieve_linked_data() to retrieve this data for the user, they'd pass the full url.
2. General Choices
   ...
   ```
   7. See the new vignette converting-an-existing-script.html#additions-to-the-r-package-1  I wrote a function that uses the API to determine date ranges for a query returning more than 10,000 rows.  I think it should be added to the R package, but I wasn't sure if it's a good idea..
...
3. Or in cast-pv-data offer an option to strip off the HATEOAS links?  Is this meaningful to the user: "cpc_subgroup": "https://search.patentsview.org/api/v1/cpc_subgroup/G01S7:4811/" or do they just want the value G01S7:4811 or even G01S7/4811?
...
8. We probably should remove printing of the url to stderr in search-pv but it's been so darn useful durning development
9. Add a method that would iterate through the data ranges generated by tell_us (in the converting-an-existing-script vignette) and return the concatenated results.  Or just wrap the whole thing, user passes in the query, we call tell_us and give them back the combined data set.
10. we probably don't need this in process-error.R though the location endpoint is not on the test server yet
...
3. The screenshot of the highcharter plot in the top-assignees vignette is incomplete.  The png only shows a little bit of IBM's yearly patents, not the full awesomeness of highcharter's plot.
&nbsp;
&nbsp;
