...
As noted below, the new version ofthe R package handles some of the changes for users.
...
Listed here are the API changes that matter to users of the original verision of the R package (which used the original version 
...
fields wil be returned by the API.  This would be simlar to get_fields("patent", groups=c("assignees")) except that it's the API deciding what fields to return
...
[API changes](#summary-of-the-api-changes) outlined above and the [Other API Changes blow](#other-api-changes)
