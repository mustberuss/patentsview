...
#> 
#> total_hits = 8,123,346
```
Also note that we did not specify the fields parameter.  In the past the API returned patent_id,
patent_title and patent_date while now it only returned patent_id.
...
* Two methods were added
   ...
   + In custom paging involving patent_id, its value needs to be padded with zeroes, see `pad_patent_id()`
...
* These options parameters went away: matched_subentities_only, subent_cnts, per_page and page
(paging now uses `after` instead of `page` and `size` instead of `per_page`)
...
ask the patent endpoint for it once we have the smallest patent_id from g_brf_sum_text.
    ...
    ```
    So, at the time of this writing, both endpoints appear to have data going back to 2023.
...
#>   ..$ cpc_class      : chr "https://search.patentsview.org/api/v1/cpc_class/G01/"
#>   ..$ cpc_subclass   : chr "https://search.patentsview.org/api/v1/cpc_subclass/G01S/"
...
#> List of 1
#>  $ cpc_groups:'data.frame':	1 obs. of  4 variables:
#>   ..$ cpc_class      : chr "https://search.patentsview.org/api/v1/cpc_class/A01/"
#>   ..$ cpc_subclass   : chr "https://search.patentsview.org/api/v1/cpc_subclass/A01B/"
...
#> [1] "https://search.patentsview.org/api/v1/assignee/b06175c5-768b-43af-bb65-1978370e6ce2/"
...
```
  ...
  # have to be a vector of all the sort fields' last values.
...
#> List of 1
#>  $ patents:'data.frame':	1000 obs. of  3 variables:
#>   ..$ patent_id   : chr [1:1000] "10632101" ...
#>   ..$ patent_title: chr [1:1000] "Methods and compositions for supporting end"..
...
#> List of 1
#>  $ patents:'data.frame':	1000 obs. of  3 variables:
#>   ..$ patent_id   : chr [1:1000] "D792527" ...
#>   ..$ patent_title: chr [1:1000] "Dog" ...
#>   ..$ patent_date : chr [1:1000] "2017-07-18" ...
#> 
#> $query_results
#> #### Distinct entity counts across all downloadable pages of output:
#> 
...
#> 
#> List of 1
#>  $ g_brf_sum_texts:'data.frame':	68 obs. of  2 variables:
#>   ..$ patent_id   : chr [1:68] "12020165" ...
#>   ..$ summary_text: chr [1:68] "TECHNICAL FIELD\n\nThe technical field genera"..
#> 
#> $query_results
#> #### Distinct entity counts across all downloadable pages of output:
#> 
