...
however an API key is now required.  The Patentsview API team has
shutdown the original version of the API so
the original version of the R package stopped working.

The original version of the R package is available on CRAN with the new version available on 
[r-universe](https://mustberuss.r-universe.dev/ui#package:patentsview).
After the PR to update R package is completed it
