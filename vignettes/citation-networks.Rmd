...
# are no longer available from the patents endpoint
...
res <- search_pv(citing_query,
  ...
  endpoint = "patent/us_patent_citations", method = "POST", per_page = 1000
)
...
res_lst2
#> List of 1
#>  $ us_patent_citations:'data.frame':	893 obs. of  2 variables:
#>   ..$ patent_id         : chr [1:893] "10007946" ...
#>   ..$ citation_patent_id: chr [1:893] "7716226" ...
```

# Count the distinct patent_id and cited_patent_id

There are only 11 PCA patents. These patents cite 1037 patents and are cited by 893 patents. Let's visualize the citations among the PCA patents. We'll create our visualization using the `visNetwork` package, which requires us to create a data frame of nodes and a data frame of edges.


```r

# with the API change we wouldn't have titles unless we call the patents endpoint
...
# We could call the patents endpoint to get the patent titles but we'll just set
...
rel_pats_res <- search_pv(
  ...
  all_pages = TRUE, per_page = 1000, method = "POST", endpoint = "patent/us_patent_citations"
)
