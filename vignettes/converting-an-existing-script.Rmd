...
## Necesary Script Changes

If you have a script that worked with the original R package and original version of the api, chances are it will need some possibly substantial changes before it will work with the new version of the R package and sapi.

## The One Required Change
First off you'll need to request an api key and then set the environmental variable PATENTSVIEW_API_KEY to the value of your api key.  Ex. set PATENTSVIEW_API_KEY=My_api_key  Without a valid api key, all your calls will be rejected by the api.  There would be a small chance that that is all that is needed to convert a simple script to uses the new R package and new version of the api.
...
The fields selected in the original stript not be available from the new version's endpoints. Fields like the government interests are no longer available. Also, some attributes have new names, like name_last in the nested inventor object.  Now in the fields parameter it would be specified as "inventor.name_last" where formerly it was "inventor_last_name" when using the patents endpoint.

Also note that some field's types changed, meaning you'll need to use different operators within your query.  Ex. assignee.organization is now a full text field, formerly it was a string.  
...
Second, here's a new function that shouly probably be added to the R package.  Given a query that probably shouldn't contain patent_date, it will return a list of start dates and end dates where each date range would return less than the new limit of 10,000 rows.
...
# the patentsview database starts with 1976. we'll start lower in case they add earlier data
# allow a start date to be passed in.  the caller might want to pick up where they
# left off last time

tell_us <- function(query, start_date = '1970-01-01' ) {
   ...
   repeat{
      ...
      # we're done if the total_hit is less than 10,000
      # the full query using the current start and end date will retrieve the last of
      # the overall data set
      if(total_hits < 10000) {
         date_ranges[[pos]] = c(start_date, end_date)  # add final pair needed
         break
      }

      # is this even possible?  we already know that total_hits > 0
      if(rows == 0)
         break
...
the_answer <- tell_us(query)
#> [1] "api calls made: 29"

the_answer
#> [[1]]
#> [1] "1970-01-01" "1983-02-28"
#> 
#> [[2]]
...
#> [1] "9999 1970-01-01, 1983-02-28"
#> [1] "9981 1983-03-01, 1989-02-13"
#> [1] "9967 1989-02-14, 1993-09-06"
#> [1] "9963 1993-09-07, 1998-02-23"
#> [1] "9973 1998-02-24, 2001-01-15"
#> [1] "The api's requests per minute limit has been reached.  Pausing for 35 seconds before continuing."
...
#> [1] "3984 2021-04-27, 2022-07-12"
```

## Conclusion

So there you have it, our attempt at listing what's changed and what to do about it.  Request your api and get going with the new R package for the new version of the api!

> Probably should steer them to the instructions on loading the updated package here.
> 
> Possible idea: ask the patentsview people if they could create a separate category for the R package in their forum.  Guessing people may need help with their conversions!
