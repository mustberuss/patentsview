...
#> [1] "https://search.patentsview.org/api/v1/patent/?q=%7B%22_gte%22%3A%7B%22patent_date%22%3A%222007-01-01%22%7D%7D&f=&s=&o={\"size\":1000}"
#> $data
...
This call to `search_pv()` sends our query to the patent endpoint (the default). The API has 23 different endpoints, corresponding to 23 different entity types
(assignees, attorneys, brf_sum_texts, claims, cpc_classes, cpc_groups, 
cpc_subclasses, detail_desc_texts, draw_desc_texts, foreign_citations, 
inventors, ipcs, locations, otherreferences, patents, publications, 
rel_app_text_publications, rel_app_texts, us_application_citations, 
us_patent_citations, uspc_mainclasses, uspc_subclasses, wipo).[^1] 
Your choice of endpoint determines which entity your query is applied to, as well as the structure of the data that is returned (more on this in the "23 endpoints for 23 entities section"). For now, let's turn our attention to the `query` parameter. 
...
#> [1] "https://search.patentsview.org/api/v1/patent/?q=%7B%22_gte%22%3A%7B%22patent_date%22%3A%222007-01-01%22%7D%7D&f=[\"patent_id\",\"patent_title\"]&s=&o={\"size\":1000}"
result
#> $data
#> #### A list with a single data frame on patents level:
#> 
...
By default, `search_pv()` returns 1,000 records per page and only gives you the first page of results. I suggest starting with something smaller, like the `per_page` = 150 below, while you're figuring out the details of your request, such as the query you want to use and the fields you want returned. Once you have those items finalized, you can use the `per_page` argument to download up to 1,000 records per page. 
...
#> [1] "https://search.patentsview.org/api/v1/patent/?q=%7B%22_eq%22%3A%7B%22inventors.inventor_name_last%22%3A%22Chambers%22%7D%7D&f=[\"patent_id\"]&s=[{\"patent_id\":\"asc\"}]&o={\"size\":1000}"
#> [1] "https://search.patentsview.org/api/v1/patent/?q=%7B%22_eq%22%3A%7B%22inventors.inventor_name_last%22%3A%22Chambers%22%7D%7D&f=[\"patent_id\"]&s=[{\"patent_id\":\"asc\"}]&o={\"size\":1000}"
#> [1] "https://search.patentsview.org/api/v1/patent/?q=%7B%22_eq%22%3A%7B%22inventors.inventor_name_last%22%3A%22Chambers%22%7D%7D&f=[\"patent_id\"]&s=[{\"patent_id\":\"asc\"}]&o={\"size\":1000,\"after\":\"5904350\"}"
#> [1] "https://search.patentsview.org/api/v1/patent/?q=%7B%22_eq%22%3A%7B%22inventors.inventor_name_last%22%3A%22Chambers%22%7D%7D&f=[\"patent_id\"]&s=[{\"patent_id\":\"asc\"}]&o={\"size\":1000,\"after\":\"9023074\"}"
#> $data
#> #### A list with a single data frame on patents level:
#> 
#> List of 1
...
#> [1] "https://search.patentsview.org/api/v1/patent/?q=%7B%22_eq%22%3A%7B%22inventors.inventor_name_last%22%3A%22Chambers%22%7D%7D&f=[\"inventors.inventor_id\",\"inventors.inventor_city\",\"inventors.inventor_country\",\"inventors.inventor_name_first\",\"inventors.inventor_name_last\",\"inventors.inventor_sequence\",\"inventors.inventor_state\",\"patent_id\"]&s=&o={\"size\":1000}"
result
#> $data
...
#> [1] "https://search.patentsview.org/api/v1/inventor/?q=%7B%22_eq%22%3A%7B%22inventor_name_last%22%3A%22Chambers%22%7D%7D&f=[\"inventor_first_seen_date\",\"inventor_gender_code\",\"inventor_id\",\"inventor_last_seen_date\",\"inventor_lastknown_city\",\"inventor_lastknown_country\",\"inventor_lastknown_latitude\",\"inventor_lastknown_location\",\"inventor_lastknown_longitude\",\"inventor_lastknown_state\",\"inventor_name_first\",\"inventor_name_last\",\"inventor_num_assignees\",\"inventor_num_patents\",\"inventor_years.num_patents\",\"inventor_years.year\",\"inventor_years_active\"]&s=&o={\"size\":1000}"
#> $data
...
#> [1] "https://search.patentsview.org/api/v1/patent/?q=%7B%22patent_id%22%3A%225116621%22%7D&f=[\"patent_id\",\"patent_date\",\"patent_title\",\"patent_year\",\"assignees.assignee_type\"]&s=&o={\"size\":1000}"
...
#> [1] "https://search.patentsview.org/api/v1/patent/?q=%7B%22_text_any%22%3A%7B%22inventors.inventor_name_last%22%3A%22Smith%22%7D%7D&f=[\"applicants.applicant_designation\",\"applicants.applicant_name_first\",\"applicants.applicant_name_last\",\"applicants.applicant_organization\",\"applicants.applicant_sequence\",\"applicants.applicant_type\",\"applicants.location_id\",\"application.application_id\",\"application.application_type\",\"application.filing_date\",\"application.filing_type\",\"application.rule_47_flag\",\"application.series_code\",\"assignees.assignee_id\",\"assignees.assignee_city\",\"assignees.assignee_country\",\"assignees.assignee_individual_name_first\",\"assignees.assignee_individual_name_last\",\"assignees.assignee_organization\",\"assignees.assignee_sequence\",\"assignees.assignee_state\",\"assignees.assignee_type\",\"attorneys.attorney_id\",\"attorneys.attorney_name_first\",\"attorneys.attorney_name_last\",\"attorneys.attorney_organization\",\"attorneys.attorney_sequence\",\"botanic.latin_name\",\"botanic.variety\",\"cpc_at_issue.cpc_class\",\"cpc_at_issue.cpc_class_id\",\"cpc_at_issue.cpc_group\",\"cpc_at_issue.cpc_group_id\",\"cpc_at_issue.cpc_section\",\"cpc_at_issue.cpc_sequence\",\"cpc_at_issue.cpc_subclass\",\"cpc_at_issue.cpc_subclass_id\",\"cpc_at_issue.cpc_type\",\"cpc_current.cpc_class\",\"cpc_current.cpc_class_id\",\"cpc_current.cpc_group\",\"cpc_current.cpc_group_id\",\"cpc_current.cpc_section\",\"cpc_current.cpc_sequence\",\"cpc_current.cpc_subclass\",\"cpc_current.cpc_subclass_id\",\"cpc_current.cpc_type\",\"examiners.art_group\",\"examiners.examiner_first_name\",\"examiners.examiner_id\",\"examiners.examiner_last_name\",\"examiners.examiner_role\",\"figures.num_figures\",\"figures.num_sheets\",\"foreign_priority.filing_date\",\"foreign_priority.foreign_application_id\",\"foreign_priority.foreign_country_filed\",\"foreign_priority.priority_claim_kind\",\"foreign_priority.priority_claim_sequence\",\"gov_interest_contract_award_numbers.award_number\",\"gov_interest_organizations.fedagency_name\",\"gov_interest_organizations.level_one\",\"gov_interest_organizations.level_three\",\"gov_interest_organizations.level_two\",\"gov_interest_statement\",\"granted_pregrant_crosswalk.application_number\",\"granted_pregrant_crosswalk.document_number\",\"inventors.inventor_id\",\"inventors.inventor_city\",\"inventors.inventor_country\",\"inventors.inventor_name_first\",\"inventors.inventor_name_last\",\"inventors.inventor_sequence\",\"inventors.inventor_state\",\"ipcr.ipc_action_date\",\"ipcr.ipc_class\",\"ipcr.ipc_classification_data_source\",\"ipcr.ipc_classification_value\",\"ipcr.ipc_id\",\"ipcr.ipc_main_group\",\"ipcr.ipc_section\",\"ipcr.ipc_sequence\",\"ipcr.ipc_subclass\",\"ipcr.ipc_subgroup\",\"ipcr.ipc_symbol_position\",\"patent_abstract\",\"patent_cpc_current_group_average_patent_processing_days\",\"patent_date\",\"patent_detail_desc_length\",\"patent_earliest_application_date\",\"patent_id\",\"patent_num_foreign_documents_cited\",\"patent_num_times_cited_by_us_patents\",\"patent_num_total_documents_cited\",\"patent_num_us_applications_cited\",\"patent_num_us_patents_cited\",\"patent_processing_days\",\"patent_term_extension\",\"patent_title\",\"patent_type\",\"patent_uspc_current_mainclass_average_patent_processing_days\",\"patent_year\",\"pct_data.application_kind\",\"pct_data.pct_102_date\",\"pct_data.pct_371_date\",\"pct_data.pct_doc_number\",\"pct_data.pct_doc_type\",\"pct_data.published_filed_date\",\"us_related_documents.published_country\",\"us_related_documents.related_doc_kind\",\"us_related_documents.related_doc_number\",\"us_related_documents.related_doc_published_date\",\"us_related_documents.related_doc_sequence\",\"us_related_documents.related_doc_status\",\"us_related_documents.related_doc_type\",\"us_related_documents.wipo_kind\",\"us_term_of_grant.disclaimer_date\",\"us_term_of_grant.term_disclaimer\",\"us_term_of_grant.term_extension\",\"us_term_of_grant.term_grant\",\"uspc_at_issue.uspc_mainclass\",\"uspc_at_issue.uspc_mainclass_id\",\"uspc_at_issue.uspc_sequence\",\"uspc_at_issue.uspc_subclass\",\"uspc_at_issue.uspc_subclass_id\",\"wipo.wipo_field\",\"wipo.wipo_field_id\",\"wipo.wipo_sequence\",\"wipo_kind\"]&s=&o={\"size\":1000}"
res$data
#> #### A list with a single data frame (with list column(s) inside) on patents level:
#> 
#> List of 1
...
#>  $ cpc_at_issue                       :'data.frame':	7715 obs. of  8 variables:
#>   ..$ patent_id      : chr [1:7715] "10000036" ...
#>   ..$ cpc_sequence   : int [1:7715] 1 2 ...
#>   ..$ cpc_class      : chr [1:7715] "https://search.patentsview.org/api/v1/cp"..
#>   ..$ cpc_class_id   : chr [1:7715] "B32" ...
#>   ..$ cpc_subclass   : chr [1:7715] "https://search.patentsview.org/api/v1/cp"..
#>   ..$ cpc_subclass_id: chr [1:7715] "B32B" ...
#>   ..$ cpc_group      : chr [1:7715] "https://search.patentsview.org/api/v1/cp"..
