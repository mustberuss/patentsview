...
We ran into trouble since we choose patent_date as the sort field which
...
endpoint.  The code is similar to what's in the [citiation network vignette](citation-networks.html),
...
users to page through result sets.  Those attributes are replaced by `size` and `after`.
2. Your sort field(s) need to create row-wise uniqueness (hint only patent_date probably won't be good)
