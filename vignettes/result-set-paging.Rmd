...
**Note: A recent API release seems to have broken paging.**  Example 1 used to work,
where we passed the last value we received as the after parameter to search_pv.
Now we need to pad the patent_id to 8 characters, see [PVS-1435](state-of-the-api.html#big-trouble)
It's unclear if this is a bug or intentional change (i.e. whether padding is temporary or
permanent).
...
pad_patent_id <- function(patent_id) {
  ...
  # not trying too hard- may need to pad reissued or design patents
  # ex RE36479.  hoping this isn't permanent or we'll have to add pad()
  # to the r package
  while(nchar(patent_id) < 8)
  {
    patent_id <- sub("([[:alpha:]]*)([[:digit:]]+)", "\\10\\2", patent_id)
  }
  patent_id
}
