...
2. The next bullet point has a link to the downloads page that's a 404 now, fixed above.
...
> Patents offer a great opportunity for data analysis  
>   
...
3. This [press release](https://www.uspto.gov/about-us/news-updates/modern-user-friendly-patent-center-fully-replace-legacy-public-pair-system) says that patft will be going away (url of Examples of those details)  

> [United States Patent and Trademark Office](https://en.wikipedia.org/wiki/United_States_Patent_and_Trademark_Office) (USPTO) [search engine](http://patft.uspto.gov/netahtml/PTO/search-adv.htm), or figure out a way to download, parse, and model huge XML data dumps. Enter PatentsView.

4. Same problem here, the search engine link would be going way

> ## PatentsView and the [patentsview](#patentsview) package
> [PatentsView](https://datatool.patentsview.org/#viz/relationships) is one of USPTO’s new initiatives intended to increase the usability and value of patent data.

3. The [PatentsView](https://datatool.patentsview.org/#viz/relationships) link in the original post is now a 404, corrected here and above

[query](#query)

> Your query has to use the [PatentsView query language](https://patentsview.org/apis/api-query-language),   

3. The link in the original post is now a 404, working one above.
...
This needs updating, as now all fields are queryable and there are 13 endpoints (more coming, like /application):

5. The original blog requested inventor_total_num_patents, which we don't get from the patents endpoint in the new version of the API. Now we'd have to call the inventors endpoint to get this field.
...
6. The original blog's "begins(cpc_subgroup_id = 'H04L63/02')" isn't currently possible,
the API throws 500 errors when trying to search via cpc fields.  The field may be full text now (some fields switch from string to full text and vice versa) and insanely, it would have to be written 

```r
qry_funs$text_phrase("cpc_current.cpc_subgroup" = "H04L63:02") 
#> {"_text_phrase":{"cpc_current.cpc_subgroup":"H04L63:02"}}
```
...
#>   ..$ cpc_subgroup_id   : chr "H04L63/02"
#>   ..$ cpc_subgroup_title: chr "Network architectures or network communication"..
#> 
#> $query_results
#> #### Distinct entity counts across all downloadable pages of output:
#> 
...
7. The next part of the original blog needs changes, both groups have new names.
...
fields
#>  [1] "patent_number"                   "patent_year"                    
#>  [3] "assignees_at_grant.assignee_id"  "assignees_at_grant.name_first"  
#>  [5] "assignees_at_grant.name_last"    "assignees_at_grant.organization"
#>  [7] "assignees_at_grant.type"         "assignees_at_grant.city"        
#>  [9] "assignees_at_grant.state"        "assignees_at_grant.country"     
#> [11] "assignees_at_grant.latitude"     "assignees_at_grant.longitude"   
#> [13] "cpc_current.sequence"            "cpc_current.section_id"         
#> [15] "cpc_current.subsection_id"       "cpc_current.group_id"           
#> [17] "cpc_current.subgroup_id"         "cpc_current.cpc_group_title"
```
...
```
8. In the new version of the API we don't get latitude/longitude back from the patents endpoint.
...
qry
#> {"_begins":{"cpc_subgroup_id":"H04L63/02"}}
