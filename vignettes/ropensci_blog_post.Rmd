...
# Now we don't get assignee latitude/longitude from the patent endpoint
# We're supposed to get assignee_location_id but the API isn't returning them
# we'd need over 14,000 locations, impractical to call another endpoint
# anyway.
...
data <-
  pv_res$data$patents %>%
  unnest(assignees) %>%
  mutate(assignee_id = str_extract(assignee, "([\\w-]+)(?=/$)")) %>%
  select(
    assignee_id, assignee_organization, patent_id,
    assignee_lastknown_longitude, assignee_lastknown_latitude
  ) %>%
...
#> ✖ Column `assignee_lastknown_longitude` doesn't exist.
...
#> Error in UseMethod("metaData"): no applicable method for 'metaData' applied to an object of class "function"
```
...
qry <- qry_funs$text_any(cpc_group_id = "H04L63/02")
qry
#> {"_text_any":{"cpc_group_id":"H04L63/02"}}

qry <- '{"_begins":{"cpc_group_id":"H04L63/02"}}'
...
data <-
  ...
  inner_join(cpc_info$data$cpc_groups, by = c("cpc_group" = "cpc_group_id")) %>% # highlighting name inconsistency
  ungroup() %>%
  mutate(
    title = case_when(
