...
To not bury the lead, the 2024-11-06 API release seems to have **broken paging**, details [below](#big-trouble)
...
print(fieldsdf[grep("bool$|int$", fieldsdf$data_type), c("endpoint", "field", "data_type")], row.names = FALSE)
  ```
* The API team created a [Swagger UI](patentsview-breaking-change.html#online-documentation) page for the new version of the API. 
...
that cast-pv-data casts to integers.[^2] [Additional details](getting-started.html#casting-fields)
   ...
   +  Two endpoints, publication/rel_app_text and patent/|rel_app_text return the same
entity, rel_app_texts.
...
the bottom of the [understanding the api](understanding-the-api.html#unexpected-results) vignette.
   ...
   + Along the same lines, this query currently returns 38,470,760 rows when that isn't 
...
~~PVS-1109~~ This is fixed <a name="otherreference">  
...
PVS-1435  <a name="big-trouble">  
**Paging seems broken with a recent API release**  
...
subsequent requests return 399 rows when we set the 'after' parameter.
There's a work around in search-pv when all_pages = TRUE (we page without using the
after parameter) but, because of the bug, users cannot implement their own paging
using the 'after' parameter.

I think I figured out a work around:  if the after: parameter is a patent_id, it needed to 
be padded to 8 characters.  E.g. "6010555" needs to be submitted as `{"after": "06010555"}`,
"RE36479" needs to become `{"after": "RE036479"}`.  I don't know if this is a bug or feature,
i.e. if paging will be fixed to work the way it used to, or whether padding will become permanent.
...
**This added a dependency to data.table.**  There is also a [new vignette](result-set-paging.html) on the new implementation of paging

  + The patent/otherreference endpoint isn't currently working (reported as a [bug](#otherreference) above).
It isn't included in the return of get_endpoints() and has only a negative test case that will
...
[Findings/Contributor 101](#findingscontributor-101) below
    * Navigation on the vignettes could be better, the possible [tech note](patentsview-breaking-change.html) and the reworked [ropensci blog post](ropensci-blog-post.html) aren't part of the navigation.
...
the new way of paging.  Possibly just say to see the new [api paging vignette](result-set-paging.html)?
    * [Sign up](https://coveralls.io/sign-up) for coveralls and add a code coverage badge?
    * httr2 improvements
...
compare the backed up fieldsdf.csv to the new one to see if anything changed (new endpoint added
etc.) If it did change, I'll do a local build before running all of the tests to see what happens.  I'll then push any changes 
to keep the tests passing.  Quarterly or so they'll update https://patentsview.org/release-notes  Another indication of
a release is that their support page stops working for a day or two, guessing while they 
plan their next sprint.
...
Currently it's displaying 
    ...
    > We couldn’t load the content in this help center  
    > Refresh the page and try again in sometime.
...
<br />
* Notes
   + It's a good idea to review the action logs on successful builds.  There can be 
