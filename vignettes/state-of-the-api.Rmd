...
You could read a whole bunch of documentation about it but here's what's going on:
...
* On a new [string case sensitivity](api-changes.html#case-sensitivity-caveat)
...
* Interesting behavior mentioned in their Jupyter Notebook, [converted to R](understanding-the-api.html)

* API Oddities
   + some [requested fields](api-changes.html#slight-weirdness) ending with _id come back without the _id[^4] 
Ex patent endpoint request inventors.inventor_id and
assignees.assignee_id will receive inventors.inventor and assignees.assignee,
...
publication endpoint.  The former is a boolean, the latter a string that cast-pv-data casts to a boolean.[^1]

      * assignee_type is a string but looks like they're integer values[^3]
...
that cast-pv-data casts to integers.[^2] [Additional details](getting-started.html#casting-fields)
   ...
   + Most endpoints' returns are the plural form of the endpoint.  Exceptions: 

     |endpoint|returned entity|
     |--------|---------------|
     |ipc|ipcr|
     |otherreference|other_references|
     |wipo|wipo|
     |publication/rel_app_text|rel_app_text_publications| 
     (patent/rel_app_text returns rel_app_texts)

### On the plus side

* All fields can be queried now.

* A [Swagger UI](patentsview-breaking-change.html#online-documentation) page was created for the API
...
* The otherreference endpoint isn't currently working (reported as a [bug](#otherreference) below).
It is not included in the return of get_endpoints() and has only a negative test case that will
...
* A possible [tech note](patentsview-breaking-change.html) which mentions
...
* Navigation on the vignettes could be better,  [understanding-the-api](understanding-the-api.html) isn't a link in the navigation,
