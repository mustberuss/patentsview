...
1. get_fields("cpc_subclass") returns both "cpc_class" and "cpc_class_id" but "cpc_class" fails their validation
(the request throws a 400 Invalid field: cpc_subclass)

2. The publication endpoint's rule_47_flag is now requested as a boolean but all values seem to be FALSE.
...
3. Not strictly a bug, but now openapi.json has all document_numbers as strings.  Previously most were
...
4. We still cannot request or query by inventors.inventor or assignees.assignee.
...
```{r}
  ...
  result <- retrieve_linked_data(URLencode(using_padding))
...
```
This function would zero pad a patent id to 8 characters.  It may be needed if
querying or paging by patent_id with ```"pad_patent_id": true```
...
```{r}
  withdrawn <- 
   ...
   result <- retrieve_linked_data(URLencode(withdrawn))
   result$data$patents$patent_id
...
1. The API returns no rows if you also specify `"pad_patent_id": true` (since the patent_ids in q: aren't padded)
2. The R package now exposes this option by
setting exclude_withdrawn = FALSE when calling search_pv()
in case you do want withdrawn patents to be included
in your results.
