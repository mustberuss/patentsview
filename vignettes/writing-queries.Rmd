...
* The fields included in your query are _queryable_ for the endpoint (i.e., the field can be used in the user query). For example, it would make sure that `assingee_country` can be used in the query argument if you sent the above query to the patent endpoint.
...
qry_funs$lte(assignee_num_inventors.num_inventors = 10)
#> {"_lte":{"assignee_num_inventors.num_inventors":10}}
```

The remaining examples are queries intended for the patent enpoint
