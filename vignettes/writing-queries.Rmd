...
* The fields included in your query are _queryable_ for the endpoint (i.e., the field can be used in the user query). For example, it would make sure that `assignees.assignee_country` can be used in the query argument if you sent the above query to the patent endpoint.
